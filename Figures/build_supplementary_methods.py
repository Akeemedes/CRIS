...
# Current, completed corrected-solver EDFM pair, without touching live jobs.
...
for name,col in [('CRIS',C[0]),('SRDM',C[1])]:
    ...
    assert np.isclose(t,1e9)
    start=np.array(start)/1e9;end=np.array(end)/1e9
    axs[0].plot(start[good],dt[good]/1e6,'o-',ms=3,lw=.8,color=col,label=name)
    axs[0].scatter(start[~good],dt[~good]/1e6,marker='x',s=20,color='#C98A00')
    waste=[]
...
# Full-width compact methods strip; separate from Bratu's construction audit.
...
# Fresh lightweight diagnostic using final regular and degenerate lambda=.01 models.
...
# Existing million-cell matrix-free pilot. Never conflate tolerance or timing scopes.
...
ax.annotate('CPU 64',xy=(412.412,5.56787),xytext=(412,4.5),ha='center',fontsize=11,arrowprops=dict(arrowstyle='-',color=C[0]))
ax.annotate('GPU 64',xy=(343.230,5.56787),xytext=(285,6.5),ha='center',fontsize=11,arrowprops=dict(arrowstyle='-',color=C[2]))
ax.annotate('GPU 32*',xy=(34.951,8.86301),xytext=(70,9.6),fontsize=11)
...
(HERE/'supplementary_methods_manifest.json').write_text(json.dumps({'sources':sources,'findings':findings,'outputs':outputs,'page_sizes_mm':sizes,'builder_sha256':hashlib.sha256(Path(__file__).read_bytes()).hexdigest()},indent=2))
