...
//        }
//
//        void initialize_embeddings(const State& uold, double DT) {
//            auto& d = derived();
//
...
//
//                this->mA[l] -= DT / cell_volume * (!DB) * Wb;
//
//                this->mA[l] += DT / cell_volume * DB * (d.mesh.bndr_face_coef(b) * this->diff_flux_func(Wb)
//                    - (vb < 0) * vb * this->adv_flux_func(Wb));
...
//                    const double cell_volume = d.mesh.cell_volume(l);
//                    // injection: q
//                    this->mA[l] += DT / cell_volume * (q > 0) * q;
//                }
//            }
//
//
...
//
//                this->mA[l1] += DT * ((vel_12 < 0) * (-vel_12) * this->adv_flux_func(u[l2])
//                    + T_12 * this->diff_flux_func(u[l2])) / cell_volume1;
//
//                this->mA[l2] += DT * ((vel_12 > 0) * (vel_12)*this->adv_flux_func(u[l1])
//                    + T_12 * this->diff_flux_func(u[l1])) / cell_volume2;
//            }
//
//            d.X = torch::cat({ this->mAT, this->mBT * DT, this->mCT * DT }, 1);
...
//                double T_12 = d.mesh.intr_face_coef(f);
//
//                double value_12 = -DT * ((vel_12 < 0) * (-vel_12) * this->adv_flux_func.grad(u[l2])
//                    + T_12 * this->diff_flux_func.grad(u[l2])) / cell_volume1;
//
//                double value_21 = -DT * ((vel_12 > 0) * (vel_12)*this->adv_flux_func.grad(u[l1])
//                    + T_12 * this->diff_flux_func.grad(u[l1])) / cell_volume2;
