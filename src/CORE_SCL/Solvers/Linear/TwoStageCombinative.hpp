...
namespace GENSOL{
  ...
  public:
    ...
    int apply( double_type *_x, double_type *_b )
    {
	  ...
	  m_xp[blk] = _x[ m_A.ainfo.tile_offset + blk * m_A.ainfo.n_vars_per_block + m_A.ainfo.elliptic_var_id ];
      ierr = m_prec1.apply( m_xp.data(), m_bp.data() );
      project_xp( );
      calculate_residual( _b );

      std::size_t N =  m_A.N();
      m_xs.resize( N );
      ierr = m_prec2.apply( m_xs.data(), m_bs.data() );
  ...
  protected:
    ...
    restrict_operator( )
    {
      ...
      for ( std::size_t blk = 0 ; blk < m_A.ainfo.n_blocks; ++blk )
	{
	  std::size_t r = m_A.ainfo.tile_offset + blk * m_A.ainfo.n_vars_per_block + m_A.ainfo.elliptic_var_id;
	  std::size_t nnz_in_row = 0;
	  for( std::size_t inz = m_A.rowptr()[r]-m_A.offset(); 
	       inz < m_A.rowptr()[r+1]-m_A.offset();
	       ++inz )
	    {
	      std::size_t var_id = (m_A.colind()[inz] - m_A.ainfo.tile_offset - m_A.offset()) % m_A.ainfo.n_vars_per_block;
	      if ( (var_id == m_A.ainfo.elliptic_var_id) &&
		   ( (m_A.colind()[inz]-m_A.offset() >= m_A.ainfo.tile_offset ) &&
		     (m_A.colind()[inz]-m_A.offset() < (m_A.ainfo.n_blocks*m_A.ainfo.n_vars_per_block + m_A.ainfo.tile_offset) ) ) )
		{
		  // extract entry
		  ++nnz_in_row;
		  std::size_t reduced_i = std::floor( ( m_A.colind()[inz] - m_A.offset() - m_A.ainfo.tile_offset)/m_A.ainfo.n_vars_per_block ) + m_A.offset();
    ...
    restrict_rhs( double_type *_b )
    {
      ...
      for ( std::size_t blk = 0 ; blk < m_A.ainfo.n_blocks; ++blk )
	{
	  m_bp[blk] = _b[ m_A.ainfo.tile_offset + blk * m_A.ainfo.n_vars_per_block + m_A.ainfo.elliptic_var_id ];
	}
      return 0;
    }
	...
	m_xp_long[m_A.ainfo.tile_offset + i*m_A.ainfo.n_vars_per_block+m_A.ainfo.elliptic_var_id] = m_xp[i];
    }
